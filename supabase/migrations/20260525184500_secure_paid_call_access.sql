...
begin
  if exists (
    ...
    where table_schema = 'public'
      and table_name = 'call_unlocks'
      and column_name = 'amount_paid_usdc'
  ) then
    execute $function$
          ...
          and exists (
            ...
            where cu.call_id = _call_id
              ...
              and (
                ...
                or coalesce(c.price_usdc, 0) <= 0
                or (
                  coalesce(cu.amount_usdc, 0) >= coalesce(c.price_usdc, 0)
                  and exists (
                    select 1
                    from public.circle_transactions tx
                    where tx.user_id = _user_id
...
-- Several RLS policies call has_role during normal browser reads. Keep this
-- grant in the hardening migration so Lovable/Supabase environments that
-- applied the earlier revoke do not fail policy evaluation.
...
-- Role writes should be service-role/server controlled. The signup trigger may
-- grant only the base "user" role; no browser-authenticated path should grant
-- admin or moderator.
