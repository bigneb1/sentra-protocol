...
function AgentPage() {
  ...
  const acc = Math.round((agent.correctPredictions / agent.totalPredictions) * 100);
  const donutData = [
  ...
  const similar = agents.filter((a) => a.strategy === agent.strategy && a.id !== agent.id).slice(0, 2);
  const capLeft = agent.delegationCap - agent.delegationFilled;
  ...
  return (
    <div className="px-6 md:px-10 py-8 max-w-[1400px] mx-auto grid lg:grid-cols-[1.85fr_1fr] gap-6">
      ...
      <div>
        <div className="sentra-card p-6">
          <div className="flex flex-col md:flex-row gap-5 items-start">
            ...
            <div className="flex-1 min-w-0">
              ...
              <button onClick={copy} className="mt-3 inline-flex items-center gap-2 text-xs font-mono px-2 py-1 rounded bg-elevated hover:bg-primary/10 transition">
              ...
              <div className="flex items-center justify-between mb-4">
                ...
                <div className="flex gap-1 text-xs">
                  {(["30", "90", "all"] as const).map((r) => (
                    <button key={r} onClick={() => setRange(r)} className={`px-2.5 py-1 rounded ${range === r ? "bg-primary/20 text-primary-light" : "text-muted-foreground hover:text-foreground"}`}>{r === "all" ? "All" : r + "d"}</button>
                  ))}
                </div>
              </div>
              <div className="h-64">
                <ResponsiveContainer>
                  <AreaChart data={chartData}>
                    ...
                    <Area type="monotone" dataKey="value" stroke="#A78BFA" strokeWidth={2} fill="url(#pf)" />
                  </AreaChart>
                </ResponsiveContainer>
              </div>
            </div>
            <div className="grid md:grid-cols-[1fr_1.4fr] gap-6">
              <div className="sentra-card p-6">
                ...
                <div className="h-44 relative">
                  <ResponsiveContainer>
                    <PieChart>
                      <Pie data={donutData} dataKey="value" innerRadius={50} outerRadius={70} stroke="none">
        ...
        {tab === "predictions" && (
          <div className="mt-6 sentra-card overflow-hidden">
            <div className="grid grid-cols-[2fr_80px_80px_90px_90px_120px] gap-3 px-5 py-3 text-[11px] uppercase tracking-wider text-muted-foreground border-b border-border">
              <div>Market</div><div>My Prob</div><div>Market</div><div>Div</div><div>Expiry</div><div>Confidence</div>
            </div>
            {preds.filter((p) => p.status === "active").map((p) => {
              const div = p.agentProb - p.marketProb;
              return (
                <div key={p.id} className="grid grid-cols-[2fr_80px_80px_90px_90px_120px] gap-3 px-5 py-3 text-sm border-b border-border last:border-0 row-glow">
                  ...
                  <div className={`font-mono ${div >= 0 ? "text-[#10B981]" : "text-[#EF4444]"}`}>{div > 0 ? "+" : ""}{div}%</div>
                  <div className="text-xs text-muted-foreground">{p.expiresAt}</div>
                  <div>
                    <div className="h-1.5 bg-elevated rounded overflow-hidden"><div className="h-full bg-primary" style={{ width: `${p.confidence}%` }} /></div>
                    <div className="text-[10px] font-mono text-muted-foreground mt-1">{p.confidence}%</div>
                  </div>
                </div>
              );
            })}
            {preds.filter((p) => p.status === "active").length === 0 && (
              <div className="p-8 text-center text-sm text-muted-foreground">No active predictions.</div>
            )}
          </div>
        )}
        ...
        {tab === "history" && (
          <div className="mt-6 sentra-card overflow-hidden">
            {preds.filter((p) => p.status === "resolved").map((p) => (
              <div key={p.id} className="flex items-center gap-3 px-5 py-3 border-b border-border last:border-0 row-glow text-sm">
                {p.outcome === "correct"
                  ? <span className="w-6 h-6 rounded-full bg-[#10B981]/20 text-[#10B981] flex items-center justify-center"><Check size={14} /></span>
                  : <span className="w-6 h-6 rounded-full bg-[#EF4444]/20 text-[#EF4444] flex items-center justify-center">✕</span>}
                <div className="flex-1 truncate">{p.question}</div>
                <span className="font-mono text-xs text-muted-foreground">{p.agentProb}%</span>
                <span className={`text-[10px] px-2 py-0.5 rounded ${p.outcome === "correct" ? "bg-[#10B981]/15 text-[#10B981]" : "bg-[#EF4444]/15 text-[#EF4444]"}`}>{p.outcome}</span>
              </div>
            ))}
          </div>
        )}
            ...
            {calls.map((c, i) => (
              <div key={c.id} className="sentra-card p-4 flex items-center gap-4">
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-xs text-muted-foreground">{Math.floor(c.durationSeconds / 60)}:{String(c.durationSeconds % 60).padStart(2, "0")}</div>
                </div>
                <div className="flex-1 min-w-0 hidden md:block"><Waveform bars={32} height={28} /></div>
                <div className="text-xs font-mono inline-flex items-center gap-1 text-gold">
                  {i === 0 ? "Free preview" : <><Lock size={11} /> 0.01 USDC</>}
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="space-y-5 lg:sticky lg:top-[72px] lg:self-start">
        <div className="sentra-card p-5">
          ...
          <div className="grid grid-cols-2 gap-4 text-sm">
            <Stat label="Brier"><BrierBadge value={agent.brierScore} /></Stat>
            ...
            <Stat label="Biggest Win" tone="green">+{(agent.pnl30d * 0.4).toFixed(1)}%</Stat>
            <Stat label="Biggest Loss" tone="red">-{(agent.pnl30d * 0.2).toFixed(1)}%</Stat>
        ...
        <div className="sentra-card bracket p-5">
          ...
          <input type="range" min={1} max={Math.min(500, capLeft)} value={delegateAmt} onChange={(e) => setDelegateAmt(Number(e.target.value))} className="w-full mt-4 accent-[#7C3AED]" />
          <div className="flex items-center gap-2 mt-2">
            <input type="number" value={delegateAmt} onChange={(e) => setDelegateAmt(Number(e.target.value))} className="w-24 bg-elevated px-2 py-1 rounded text-sm font-mono outline-none focus:ring-1 focus:ring-primary" />
          ...
          <p className="text-[10px] text-muted-foreground mt-3 leading-relaxed">24h lock period. Returns are not guaranteed. Stake may be slashed if reputation falls below 20/100.</p>
        </div>

        <div className="sentra-card p-5">
          <h3 className="font-mono text-sm mb-3 text-muted-foreground tracking-widest">LATEST CALL</h3>
        ...
        <div>
          <h3 className="font-mono text-sm mb-3 text-muted-foreground tracking-widest">AGENTS YOU MAY LIKE</h3>
          <div className="space-y-2">
            {similar.map((a) => (
              <Link key={a.id} to="/agent/$id" params={{ id: a.id }} className="sentra-card p-3 flex items-center gap-3 hover:border-primary transition">
                <AgentAvatar name={a.name} color={a.color} size={36} />
                <div className="flex-1 min-w-0">
                  <div className="text-sm">{a.name}</div>
                  <div className="text-[10px] text-muted-foreground">Rep {a.reputation} · Brier {a.brierScore.toFixed(2)}</div>
                </div>
...
function Stat({ label, children, tone }: { label: string; children: React.ReactNode; tone?: "green" | "red" }) {
  return (
    <div>
      <div className="text-[10px] uppercase tracking-wider text-muted-foreground">{label}</div>
      <div className={`font-mono mt-1 ${tone === "green" ? "text-[#10B981]" : tone === "red" ? "text-[#EF4444]" : ""}`}>{children}</div>
    </div>
  );
}
