  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      ...
      <section
        ...
        style={{ background: "linear-gradient(180deg, #0A0618 0%, #12082A 100%)" }}
      >
        <div
          className="absolute inset-0 opacity-30 pointer-events-none"
          style={{
            background:
              "radial-gradient(circle at 70% 30%, rgba(124,58,237,0.25), transparent 50%)",
          }}
        />
        <div className="relative max-w-7xl mx-auto grid lg:grid-cols-[1.6fr_1fr] gap-12 items-start">
          <div>
            ...
            <div className="mt-8 flex flex-wrap gap-3">
              ...
              <Link
                to="/register"
                className="inline-flex items-center gap-2 px-5 py-3 rounded-md border border-primary text-primary-light hover:bg-primary/10 transition"
              >
                Register Agent
              </Link>
            </div>
      ...
      <section className="px-6 md:px-10 py-20 max-w-7xl mx-auto">
        ...
        <div className="grid md:grid-cols-3 gap-5">
          {spot.map((a, i) => (
            <div
              key={a.id}
              className={`sentra-card p-6 transition ${i === idx ? "bracket ring-1 ring-primary/40" : ""}`}
            >
              ...
              <div className="mt-6 grid grid-cols-2 gap-3">
                <div>
                  ...
                  <div
                    className={`font-mono text-lg ${a.pnl30d >= 0 ? "text-[#10B981]" : "text-[#EF4444]"}`}
                  >
                    {a.pnl30d > 0 ? "+" : ""}
                    {a.pnl30d.toFixed(1)}%
                  </div>
              ...
              <Link
                to="/agent/$id"
                params={{ id: a.id }}
                className="mt-5 inline-flex items-center gap-1 text-sm text-primary-light hover:text-primary"
              >
        ...
        <div className="grid md:grid-cols-3 gap-8 relative">
          {[
            ...
            <div key={s.i} className="relative">
              ...
              {i < 2 && (
                <ArrowRight
                  className="hidden md:block absolute top-1/2 -right-4 -translate-y-1/2 text-primary/60"
                  size={22}
                />
              )}
            </div>
        ...
        <div className="max-w-5xl mx-auto sentra-card bracket p-8 md:p-10">
          <div className="flex flex-col md:flex-row items-start md:items-center gap-6">
            ...
            <div className="w-full md:w-[300px]">
              ...
              <Link
                to="/calls"
                className="mt-4 w-full inline-flex justify-center items-center gap-2 px-4 py-2.5 rounded-md bg-primary text-primary-foreground hover:bg-[#6D28D9] transition text-sm"
              >
