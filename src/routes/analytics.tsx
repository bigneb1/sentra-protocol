  ...
  head: () => ({
    meta: [
      { title: "Analytics — SENTRA" },
      {
        name: "description",
        content:
          "Agent leaderboard, accuracy trends, strategy heatmap, and PnL breakdowns across the SENTRA protocol.",
      },
    ],
  }),
...
function Analytics() {
  ...
  const heatmap = useMemo(() => {
    ...
    const grouped = strategies.map((strat) => {
      ...
      const acc =
        inGroup.reduce(
          (s, a) => s + (a.totalPredictions ? a.correctPredictions / a.totalPredictions : 0),
          0,
        ) / n;
    ...
    <div className="px-4 md:px-10 py-6 md:py-8 max-w-[1400px] mx-auto">
      <div className="mb-6 flex flex-col md:flex-row md:items-end md:justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground mt-1 text-sm">
            Protocol-wide intelligence. Live from Arc Testnet.
          </p>
        </div>
      ...
      {!hasData && (
        <div className="sentra-card p-6 mb-6 border-dashed border-2 border-primary/30 text-center">
          ...
          <p className="text-xs text-muted-foreground max-w-md mx-auto">
            Agents are registered but haven't submitted scored predictions. Stats populate live as
            predictions resolve on-chain.
          </p>
          <Link
            to="/register"
            className="mt-4 inline-block px-4 py-2 rounded-md bg-primary text-primary-foreground hover:bg-[#6D28D9] text-sm"
          >
            Register an agent
          </Link>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-3 mb-6">
        {[
          {
            l: "TVL Delegated",
            v: `$${totals.delegated.toLocaleString()}`,
            icon: DollarSign,
            c: "#7C3AED",
          },
          ...
          return (
            <div key={k.l} className="sentra-card p-4">
              ...
              <div className="font-mono text-lg md:text-xl mt-2" style={{ color: k.c }}>
                {k.v}
              </div>
            </div>
          );
        ...
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <div className="font-mono text-sm mt-1 text-muted-foreground">
              Normalized metric per strategy · {windowDays}d window
            </div>
          </div>
        </div>
        <div className="overflow-x-auto">
          <div className="min-w-[520px]">
            <div
              ...
              style={{ gridTemplateColumns: `120px repeat(${heatmap.metrics.length}, 1fr)` }}
            >
              <div />
            ...
            <div className="flex items-center gap-3 mt-4 text-[10px] text-muted-foreground">
              ...
              <div
                ...
                style={{
                  background:
                    "linear-gradient(90deg, rgba(124,58,237,0.08), rgba(124,58,237,0.65))",
                }}
              />
              <span>High</span>
            </div>
        ...
        <div className="p-5 border-b border-border flex items-center justify-between">
          <div>
            ...
            <div className="font-mono text-sm mt-1">
              Full breakdown · sorted by {windowDays}d PnL
            </div>
          </div>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-sm min-w-[760px]">
            ...
            <tbody>
              {comparison.map((a) => (
                <tr
                  key={a.id}
                  className="border-b border-border/50 hover:bg-elevated/40 transition"
                >
                  <td className="p-3">
                    <Link
                      to="/agent/$id"
                  ...
                  <td className="p-3 text-right font-mono">
                    {a.brierScore ? a.brierScore.toFixed(3) : "—"}
                  </td>
                  <td className="p-3 text-right font-mono">
                    {a.sharpeRatio ? a.sharpeRatio.toFixed(2) : "—"}
                  </td>
                  <td className="p-3 text-right font-mono">
                    {a.winRate ? (a.winRate * 100).toFixed(0) + "%" : "—"}
                  </td>
                  <td
                    className={`p-3 text-right font-mono ${a.pnl7d > 0 ? "text-[#10B981]" : a.pnl7d < 0 ? "text-[#EF4444]" : "text-muted-foreground"}`}
                  >
                    {a.pnl7d ? `${a.pnl7d > 0 ? "+" : ""}${a.pnl7d.toFixed(1)}%` : "—"}
                  </td>
                  <td
                    className={`p-3 text-right font-mono ${a.pnl30d > 0 ? "text-[#10B981]" : a.pnl30d < 0 ? "text-[#EF4444]" : "text-muted-foreground"}`}
                  >
      ...
      <div className="sentra-card overflow-hidden mb-6">
        <div className="p-5 flex items-center justify-between border-b border-border">
          ...
          <Link
            to="/arena"
            className="text-xs text-primary-light hover:text-primary inline-flex items-center gap-1"
          >
          ...
          <div className="divide-y divide-border">
            {leaderboard.map((a, i) => (
              ...
              >
                ...
                <div>
                  <ReputationRing value={a.reputation} size={30} showLabel={false} />
                </div>
                <div className="font-mono text-xs">
                  {a.brierScore ? a.brierScore.toFixed(3) : "—"}
                </div>
                <div
                  className={`font-mono text-xs ${a.pnl7d >= 0 ? "text-[#10B981]" : "text-[#EF4444]"}`}
                >
                  {a.pnl7d ? `${a.pnl7d > 0 ? "+" : ""}${a.pnl7d.toFixed(1)}%` : "—"}
                </div>
                <div
                  className={`font-mono text-xs ${a.pnl30d >= 0 ? "text-[#10B981]" : "text-[#EF4444]"}`}
                >
                  {a.pnl30d ? `${a.pnl30d > 0 ? "+" : ""}${a.pnl30d.toFixed(1)}%` : "—"}
                </div>
              </Link>
        ...
        <div className="space-y-2">
          {recentResolved.map((p) => {
            ...
            return (
              ...
              >
                <span
                  className={`w-1.5 h-1.5 rounded-full ${p.outcome === "correct" ? "bg-[#10B981]" : "bg-[#EF4444]"}`}
                />
                <span className="text-muted-foreground font-mono truncate hidden md:inline w-32">
                  {agent?.name}
                </span>
                <span className="flex-1 truncate">{p.question}</span>
                <span className="font-mono text-muted-foreground">{p.agentProb}%</span>
                <span
                  className={`font-mono ${p.outcome === "correct" ? "text-[#10B981]" : "text-[#EF4444]"}`}
                >
                  {p.outcome === "correct" ? "WIN" : "LOSS"}
                </span>
              </div>
            );
          })}
