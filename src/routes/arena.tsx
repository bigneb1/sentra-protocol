...
const stratFilters: ("All" | Strategy)[] = [
  "All",
  "Macro",
  "Sports",
  "Contrarian",
    ...
    <div className="px-6 md:px-10 py-8 max-w-[1400px] mx-auto">
      ...
      <div className="flex flex-col md:flex-row gap-3 mb-5">
        <select
          ...
          className="sentra-card px-3 py-2 text-sm bg-card outline-none focus:border-primary"
        >
          {sortOptions.map((o) => (
            ...
            return (
              <div key={a.id} className="border-b border-border last:border-0">
                ...
                >
                  ...
                  <div>
                    <ReputationRing value={a.reputation} size={36} showLabel={false} />
                  </div>
                  <div>
                    <BrierBadge value={a.brierScore} />
                  </div>
                  <div
                    className={`font-mono ${a.pnl7d >= 0 ? "text-[#10B981]" : "text-[#EF4444]"}`}
                  >
                    {a.pnl7d > 0 ? "+" : ""}
                    {a.pnl7d.toFixed(1)}%
                  </div>
                  <div
                    className={`font-mono ${a.pnl30d >= 0 ? "text-[#10B981]" : "text-[#EF4444]"}`}
                  >
                    {a.pnl30d > 0 ? "+" : ""}
                    {a.pnl30d.toFixed(1)}%
                  </div>
                  ...
                  <div className="flex items-center justify-end gap-2">
                    <Link
                      ...
                      className="text-primary-light text-xs hover:text-primary inline-flex items-center gap-1"
                    >
                ...
                {open && (
                  <div className="px-5 pb-5 grid md:grid-cols-[1fr_180px_1fr] gap-5 bg-elevated/30">
                      ...
                      <ul className="space-y-2">
                        {preds.map((p) => (
                          <li key={p.id} className="flex items-center gap-2 text-xs">
                            <span
                              className={`w-1.5 h-1.5 rounded-full ${p.outcome === "correct" ? "bg-[#10B981]" : p.outcome === "wrong" ? "bg-[#EF4444]" : "bg-primary-light"}`}
                            />
                    ...
                    <div>
                      ...
                      <Link
                        to="/calls"
                        className="text-xs text-primary-light inline-flex items-center gap-1 mt-2"
                      >
